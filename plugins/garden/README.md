...
- `garden-reviewer` is the isolated, read-only reviewer used after deterministic gates
  or when the implementer must not review its own work.
...
and `.mcp.json`. Codex discovers `.codex-plugin/plugin.json`, `agents/openai.yaml`, the
same Markdown agent, skills, hooks, and MCP config.

The shared MCP launcher accepts both path models. Claude Code supplies
`CLAUDE_PLUGIN_ROOT`; Codex resolves `cwd: "."` from the installed plugin root. Both
start the dependency-free Python server in `tools/garden_mcp.py`.
...
same `assets/garden-rules.md` content in `.claude/rules/garden.md` and/or a marked
project-root `AGENTS.md` block.
...
```sh
python3 -m unittest discover -s plugins/garden/tests -v
python3 /path/to/plugin-creator/scripts/validate_plugin.py plugins/garden
